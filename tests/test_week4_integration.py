...
def test_vault_audit_log_emitted(vault_env) -> None:
    """全 access が audit log に literal record (ADR-007 layer 7)。"""
    import json
...
def test_produce_video_script_structure() -> None:
    """produce_video.py の structure literal verify (file scan、 TTS engine runtime は user-side)。"""
    pv_path = PJ_ROOT / "scripts" / "produce_video.py"
    content = pv_path.read_text(encoding="utf-8")
    # cross-PJ universal video-pipeline SSoT 順守 markers (T2 SSoT pattern literal sync、 Task 2 で Phase 2-3 back-port 完遂)
    assert "_scenes_factory" in content
    assert "SCENES = _scenes_factory()" in content
    assert "888753763" in content  # まお おちついた speaker_id literal
    assert "ENGINE_URL" in content  # T2 SSoT naming literal sync (旧 AIVIS_URL から rename)
    assert "aivis_synthesize" in content
    assert "video-pipeline" in content
    # Phase 2-3 back-port marker (T2 literal inherit、 prior session で foundation のみだった部分が full pipeline 化)
    assert "record_demo" in content  # Phase 2: Playwright recording
    assert "compose_final" in content  # Phase 3: ffmpeg compose + 字幕 burn-in + クレジット overlay
    assert "generate_srt" in content  # 字幕 SRT auto-gen
    assert "make_padded_wav" in content  # lead/trail silence pad
    assert "concat_narration" in content  # 16 scene narration concat
    # 16 SCENES literal 列挙確認
    for i in range(1, 17):
        assert f'"S{i}"' in content, f"S{i} scene missing in produce_video.py"
