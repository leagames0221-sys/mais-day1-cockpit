"""Week 2 day 4-7: LangGraph state graph + 4 軸 agent parallel + e2e smoke (ADR-204)。"""
...
def test_dimension_to_jp_axes_mapping_literal() -> None:
    """ADR-204 + ADR-205 dimension ↔ axis literal mapping。"""
    assert "union_relation" in DIMENSION_TO_JP_AXES["organization"]
    assert "family_integration" in DIMENSION_TO_JP_AXES["culture"]
    assert DIMENSION_TO_JP_AXES["system"] == []  # ADR-205: system 軸は jp_day1 と直接 link なし
...
def test_t3_graph_e2e_smoke_with_mock_t2() -> None:
    """T2 mock output → T3Output 全 pipeline e2e literal run (ADR-204 完成形)。"""
    app = build_t3_graph(use_checkpoint=True)
...
def test_t3_graph_returns_valid_t3output_schema() -> None:
    """LangGraph 出力が ADR-202 T3Output Pydantic schema を literal pass。"""
    app = build_t3_graph(use_checkpoint=True)
    initial_state = {
        "t2_output": _minimal_t2(),
        "industry": "製造業",
