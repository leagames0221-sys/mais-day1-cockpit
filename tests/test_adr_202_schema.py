"""ADR-202 schema test (src/integration/schemas.py が SSoT、 本 test は instantiate + validation 検証)。
...
def test_t2_output_schema_instantiates() -> None:
    """T2 → T3 入力 schema が valid placeholder data で literal 構築できる (ADR-202)。"""
    sample = T2Output(
        ddp_id="DDP-000001",
        citations=[
            Citation(
...
def test_t3_output_schema_instantiates() -> None:
    """T3 → T4 出力 schema が valid placeholder data で literal 構築できる (ADR-202)。"""
    sample = T3Output(
        ip_id="IP-000001",
        plan_nodes=[
            PlanNode(
...
def test_id_prefix_validation_rejects_invalid() -> None:
    """正規表現 pattern が invalid prefix を literal reject (ADR-203)。"""
...
def test_t2_t3_citation_array_link_back() -> None:
    """T3 CommunicationKit が T2 CIT prefix を citation_array literal で参照可能 (ADR-202)。"""
...
def test_audience_specific_redaction_literal_validates() -> None:
    """ADR-203 audience-specific redaction matrix、 5 audience 全 literal validate。"""
...
def test_jp_day1_5_axes_literal_validate() -> None:
    """ADR-205 jp_day1 5 軸 全 literal validate。"""
