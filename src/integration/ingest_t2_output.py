"""T2 API output → T3 IntegrationPlan 変換 + jp_day1 trigger mapping。

 T2 → T3 trigger mapping:
...
def map_jp_pattern_to_axes(pattern_type: str) -> list[JPDay1Axis]:
    """T2 jp_patterns_hits の pattern_type を T3 jp_day1 5 軸に literal map。"""
        ...
        for axis in map_jp_pattern_to_axes(jpp.pattern_type):
            hits.append(
                JPDay1Pattern(
                    ...
                    summary_redacted=f"T2 jp_pattern {jpp.pattern_type} hit、 軸 {axis} に literal map",
...
def ingest_t2_output_to_ip(
    ...
    NOTE: 4 軸 PlanNode + DependencyEdge + RiskScore は LangGraph state graph で生成、
