"""T3 Object Type 6 件 + T2 → T3 入出力契約 schema (ADR-202 + ADR-203 accepted literal 実装)。

SSoT: mais-deal-matching internal ADR § 3 (PII/Op 分離) + T2 ADR-102 (Citation schema) + T3 ADR-202/203。
...
# ============================================================
# ADR-202: T2 → T3 入力 schema (T2 ADR-102 actual schema literal 整合)
# ============================================================


class Citation(BaseModel):
    """T2 ADR-102 から literal 継承 (section_header は ChunkMetadata 側 field のため Citation には不在)。"""
...
class T2Output(BaseModel):
    """T2 API output (T3 入力 source、 ADR-202 schema)。"""
...
# ============================================================
# ADR-203: T3 Object Type 6 件 (operational layer)
# ============================================================
...
class JPDay1Pattern(BaseModel):
    """5 軸 detector 発火イベント (ADR-205 pattern library)。"""
