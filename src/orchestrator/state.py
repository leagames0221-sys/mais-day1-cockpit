"""LangGraph state schema (ADR-204 4 軸 agent parallel + 6 軸 mitigation literal 反映)。

ADR-204 6 軸 mitigation 設計反映:
...
class T3State(TypedDict, total=False):
    """T3 orchestrator state schema (ADR-204 DAG flow literal 順守)。
