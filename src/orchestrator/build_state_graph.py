...
def ingest_t2_node(state: T3State) -> dict[str, Any]:
    """T2 output → IntegrationPlan + jp_day1_hits 変換 + Stage 2 LLM augmentation。"""
    ...
    # Stage 2 LLM augmentation: keyword false positive 排除 + reasoning literal 蓄積
...
def finalize_t3_output_node(state: T3State) -> dict[str, Any]:
    """T3Output を Pydantic validation 経由で literal finalize。"""
    ip = state["integration_plan"]
    out = T3Output(
        ip_id=ip.ip_id,
        plan_nodes=state.get("plan_nodes", []),
...
def build_t3_graph(use_checkpoint: bool = True):
    """ DAG literal construction (memory-only checkpoint、 6 軸 mitigation 設計反映)。
